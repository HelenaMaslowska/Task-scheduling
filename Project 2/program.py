...
# KOPIA ZAPASOWA PIERWSZEGO PROGRAMU

# for n in [50]:  ##, 100, 150, 200, 250, 300, 350, 400, 450, 500
# 	f = open("in_"+str(index) + "_" + str(n) + ".txt", "r")
# 	n = int(f.readline())
# 	machine = list(map(float, f.readline().split()))
# 	jobs = []
# 	for i in range(n):
...
# 	machine1, machine2, machine3, machine4, machine5 = [], [], [], [], []
# 	for k in range(0, int(n-4), 5):
# 		machine1.append(jobs[k])
# 		machine2.append(jobs[k+1])
# 		machine3.append(jobs[k+2])
# 		machine4.append(jobs[k+3])
# 		machine5.append(jobs[k+4])
# 	print(Fore.WHITE, "machine1:", machine1)
# 	f = open("out_" + str(from_who) + "_" + str(index) + "_" + str(n) + ".txt", "w")
	
# 	output = [machine1, machine2, machine3, machine4, machine5]
# 	result_time = count_time(output)

# 	print(Fore.WHITE, result_time)
# 	f.write(str(round(result_time, 2)) + "\n")
# 	for m in output:
# 		for j in range(len(m)):
# 			if j != len(m)-1: 	f.write(str(m[j][0]) + " ")
